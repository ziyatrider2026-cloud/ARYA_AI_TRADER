...
export function runPortfolioBacktest(series: SymbolReplay[], strategy: PortfolioStrategy, options: { initialCash: number; costs?: Partial<SimulationCosts> }): PortfolioBacktestResult {
  ...
  const byTime = new Map<number, Record<string, Candle>>();
  for (const s of series) for (const candle of s.candles) (byTime.get(candle.t) ?? byTime.set(candle.t, {}).get(candle.t)!).__proto__;
  // Build the map without relying on object prototype mutation.
  ...
  for (let i = 0; i < times.length - 1; i++) {
    const t = times[i]!; const next = times[i + 1]!; const visible: Record<string, Candle> = {};
    for (const s of series) { const c = s.candles.find((x) => x.t === t); if (c) visible[s.symbol] = c; }
    ...
    for (const [symbol, candle] of Object.entries(timeline.get(next) ?? {})) marks[symbol] = candle.close;
  ...
  const finalMarks = Object.fromEntries(Object.entries(timeline.get(times[times.length - 1]!) ?? {}).map(([s, c]) => [s, c.close]));
